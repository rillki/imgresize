...
import std.stdio : write, writef, readln;
...
/++
 ...
 +   defaultYes = default behaviour upon empty or invalid input
 + Returns: `true` for yes, `false` for no.
 +/
bool askYesNo(in string question, in bool defaultYes = true)
{
    // write question
    ...
    // handle empty input (just pressed Enter)
    if (input.length == 0)
    {
        return defaultYes;
    }
    ...
    else return defaultYes; // invalid input, use default
}
